...
package ai.safekids.httpproxy.handler;
...
public class TailBackendHandler extends ChannelDuplexHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(TailBackendHandler.class);
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        LOGGER.debug("{} : read {} from server", connectionContext, LogWrappers.description(msg));
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
        if (this.ignoreException(cause)) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                    "{} Swallowing a harmless 'connection reset by peer / broken pipe' error that occurred while " +
                    "writing close_notify in response to the peer's close_notify",
                    ctx.channel(), cause);
            }
        } else {
            LOGGER.error("{} : Unknown Tail Frontend Backend Handler exception", connectionContext, cause);
        }
